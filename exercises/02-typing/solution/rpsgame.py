...
rolls: dict[str, dict[str, list[str]]] = {}


# Notice that you get auto complete for things like:
# rolls['rock']['defeated_by']. <-- dot there
...
def play_game(player_1: str, player_2: str):
...
def find_winner(wins: dict[str, int], names: Iterable[str]) -> Optional[str]:
    best_of = 3
    for name in names:
...
def check_for_winning_throw(player_1: str, player_2: str, roll1: str, roll2: str) -> Optional[str]:
    if roll1 == roll2:
...
def get_roll(player_name: str, roll_names: list[str]) -> Optional[str]:
...
def record_win(winner_name: str):
    leaders = load_leaders()
